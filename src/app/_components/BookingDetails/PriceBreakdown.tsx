    ...
    <div className={'divide-y space-y-5'}>
      <div className={'space-y-3 text-sm'}>
        ...
        {booking.coupons.map((c) => (
          <div key={c.couponID + '—' + c.label} className={'text-primary font-semibold justify-between flex'}>
        ...
        {booking.guestFee && (
          <div className={'text-foreground/50 justify-between flex'}>
            <span className={''}>Extra Guest Fee</span>
            <span>{formattedPrice(booking.guestFee.total)}</span>
          </div>
        )}
        {booking.additionalFees.map(
          fee =>
            fee.total != 0 && (
              <div key={fee.priceID + '—' + fee.label} className={'text-foreground/50 justify-between flex'}>
